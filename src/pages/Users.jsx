...
import { getUsers, createUser, updateUser, deleteUser, adminResetPassword } from '../api/users';
...
export default function Users() {
  ...
  const handleAdminReset = async () => {
    ...
    setResetError('');
    try {
      await adminResetPassword(resetModal.id, resetPass);
      setResetModal(null);
    } catch (err) {
      const detail = err.response?.data?.detail;
      setResetError(typeof detail === 'string' ? detail : 'Errore nel reset');
    } finally {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Imposta una nuova password per <strong>{resetModal.first_name} {resetModal.last_name}</strong>
            </p>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full border rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 ${
                    resetPass && resetPass.length < 8 ? 'border-red-400 bg-red-50' : 'border-gray-300'
                  }`}
              ...
              {resetPass && resetPass.length < 8 && (
                <p className="text-red-500 text-xs mt-1">Minimo 8 caratteri ({resetPass.length}/8)</p>
              )}
            </div>
            ...
            <div className="flex gap-3 mt-5">
              ...
              >
                {resetLoading ? 'Salvo...' : 'Conferma reset'}
              </button>
            </div>
          </div>
        </div>
